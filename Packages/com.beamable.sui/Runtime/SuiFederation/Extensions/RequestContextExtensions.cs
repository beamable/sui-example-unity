...
namespace Beamable.Microservices.SuiFederation
{
    public static class RequestContextExtensions
    {
            ...
            try
            {
                if (requestContext.Headers.TryGetValue(HeaderEncodedRc, out var encodedRc))
                {
                    ...
                    if (rcMap is not null && rcMap.TryGetValue(RcExternalField, out var externalList))
                    {
                        ...
                        if (!string.IsNullOrEmpty(externalListJson))
                            return JsonConvert.DeserializeObject<List<ExternalIdentity>>(externalListJson,
                                JsonSerializerSettings) ?? Enumerable.Empty<ExternalIdentity>();
                    }
                }
                ...
                if (requestContext.Headers.TryGetValue(HeaderExternal, out var externalListLegacy))
                {
                    if (!string.IsNullOrEmpty(externalListLegacy))
                        return JsonConvert.DeserializeObject<List<ExternalIdentity>>(externalListLegacy,
                            JsonSerializerSettings) ?? Enumerable.Empty<ExternalIdentity>();
                }
        ...
        public static ExternalIdentity? GetExternalIdentity(this RequestContext requestContext,
            MicroserviceInfo microserviceInfo)
        {
            ...
            var existingExternalIdentity = externalIdentities
                .FirstOrDefault(x =>
                    x.providerService == microserviceInfo.MicroserviceName &&
                    x.providerNamespace == microserviceInfo.MicroserviceNamespace);
        ...
        {
            public RequestContextParsingException(string message) : base((int)HttpStatusCode.BadRequest,
                "RequestContextParsingError", message)
            {
            }
        }
    }
